...
export class ListableCollection<V, Filter> extends Collection<V> implements Types.IListableCollection<V, Filter> {
    ...
    private idExtractor: Types.IdExtractor<V>;
    private listCacheInvalidators: (filter: Filter) => string[];
    ...
    constructor(options: Types.IListableCollectionOptions<V, Filter>) {
        ...
        this.idExtractor = options.idExtractor || ((v: V) => (v as any).id);
        this.listCacheInvalidators = options.listCacheInvalidators;
    ...
    public async getListWithMeta(filter: Filter): Promise<Types.IListData<V>> {
        ...
        if (!this.listCache) {
            ...
            const invalidators = this.listCacheInvalidators ? this.listCacheInvalidators(filter) : null;
            if (
                loadedFromCache &&
                invalidators &&
                !(await this.checkInvalidators(invalidators, loadedFromCache.ts))
            ) {
                loadedFromCache = null;
            }
            if (loadedFromCache) {
                if(this.cowardListCache) {
                    // return cached list if all items stored in cache and no one item has been recached after list cache was built
            ...
            await this.listCache.set(cacheKey, {
                ids: loaded.items.map(this.idExtractor),
                meta: loaded.meta,
                ts: now,
            });

            if(invalidators) {
                await this.setInvalidators(invalidators, now);
            }
        }
    ...
    public async invalidateListCache(invalidator: string | string[]): Promise<void> {
